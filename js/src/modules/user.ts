...
export function createUserModule(client: SdkClient) {
  return {
    ...
    async getUserInfo(): Promise<UserInfoResponse> {
      const info = await client.get<UserInfoResponse>('/uc/v1/auth/info')
      if (info.session_mode) {
        client.setSessionMode(info.session_mode as SessionMode)
      }
      return info
    },
    ...
    updateProfile(data: Record<string, any>): Promise<void> {
      return client.put('/uc/v1/profile/info', data)
    },
    ...
    updateUsername(username: string): Promise<void> {
      return client.put('/uc/v1/profile/username', { username })
    },
    ...
    updatePassword(params: { newPassword: string; oldPassword?: string }): Promise<void> {
      return client.put('/uc/v1/profile/password', {
    ...
    getPasswordPolicy(): Promise<PasswordPolicy> {
      return client.get('/uc/v1/auth/password/policy')
    },
    ...
    sendEmailVerify(): Promise<void> {
      return client.post('/uc/v1/profile/email/verify/send', { app_id: client.appId })
    },

    /** 校验邮箱验证码（需登录），成功后 email_verified 置 1 */
    verifyEmail(code: string): Promise<void> {
      return client.post('/uc/v1/profile/email/verify', { code })
    },

    /** 获取当前用户菜单树，可按 app_id 过滤 */
    getMenuTree(appId?: string): Promise<MenuItem[]> {
      const params = appId ? { app_id: appId } : undefined
      return client.get('/uc/v1/menus', params)
    },
  }
}
