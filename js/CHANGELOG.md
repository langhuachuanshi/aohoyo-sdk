...
- **`auth.sendCode` / `user.sendProfileCode` 补带 `app_id`**：此前两个发码接口原样透传调用参数，
  未像 `login`/`register`/`codeLogin` 一样自动附加 `app_id`，导致应用绑定了专属短信/邮件服务商时
  发码仍走全局默认服务商（按应用路由失效）。现统一自动附加 `client.appId`，调用方无需改动。
  行为变更：初始化了 `appId` 的 SDK 发出的验证码，将改由该应用绑定的服务商发送（未绑定应用的
  行为不变，仍走全局默认）
