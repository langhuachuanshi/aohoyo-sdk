...
客户端 JS SDK（npm 包 `@aohoyo/client-sdk` v0.9.0），供 **Web / 桌面端（Tauri/Wails/Electron）/ 移动端** 集成使用。封装用户中心（UC）和管理服务（AS）的全部客户端 API，通过 JWT Bearer Token 认证。

与 [`go`](../go/AGENTS.md) 区别：client-js 是**客户端 SDK**（JWT），server-go 是**服务端 SDK**（S2S HMAC 签名）。
...
| 发布 | 打 tag `js/v*` → GitHub Actions 自动发布 npm（推送即发布） |
...
| 函数 | 说明 |
|------|------|
| `submit(params)` | 提交反馈（登录走 JWT；免登录设备走 DeviceSign，需 app_secret） |
| `myList(params?)` | 我的反馈列表 |
| `uploadImage(file)` | 反馈图片上传（`/as/v1/feedback/upload-image`） |

> cloud 模块（云变量/云函数）已于 v0.8.0 删除（服务端路由早已移除）。
...
2. 更新 `CHANGELOG.md`（`[Unreleased]` → 版本节，行为变更显式标注）
3. **README 对照检查**：新特性进能力表/快速开始，已下线 API 删除（README 管「当前面」，CHANGELOG 管「历史」）
4. `npm run build` 确保编译通过（tsc strict）
5. 打 tag `js/v{version}` 并推送（推送即触发 npm 自动发布，注意与主仓库部署节奏对齐）
