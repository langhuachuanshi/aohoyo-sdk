...
- **`GetAds` 位 code 必传**: `positionCode` 传空串不再回落「拉取该应用全部广告位」，
  改为直接报错（此前空串时不发 position 参数即全量）。位 code 是「某个客户端构建 ↔
  后台广告位配置」之间的私有约定，SDK 作为通用库无法解释「该应用全部广告位」；且服务端
  广告位含公共区合并场景，全量 map 会出现宿主从未定义的键，全量语义不可解释、还把
  「显示什么」的裁决权漏给调用方。服务端接口不动（仍支持不传 = 全量），仅 SDK 层收窄。
  同日架构决策：js `sdk.ads` 模块整体移除（@aohoyo/client-sdk 0.12.0，客户端能力收归
  原生层），广告拉取桌面端以 go/native 与 rust/native 为官方通道

### 新增

- **`GetKV`**（native）: 拉取应用可见的 KV 配置（GET /as/v1/app/kv，DeviceSign 签名，
  本应用 + 公共区合并、本应用同 key 优先），值已按类型解析。`keys` 为 variadic：
  不传返回全部，传指定键按需拉取（?key=k1,k2，不存在的键直接缺席）。
  与 js `sdk.kv.get`、rust `get_kv` 同构
